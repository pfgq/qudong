...
static unsigned long lookup_symbol_from_kallsyms(const char *target)
{
    struct file *file;
    loff_t pos = 0;
    char *buf;
    ...
    if (IS_ERR(file)) {
        printk(KERN_ERR "[kallsyms] open failed\n");
        return 0;
    }
    ...
    while ((read = kernel_read(file, buf, PAGE_SIZE - 1, &pos)) > 0) {
        char *line, *next;
        ...
        while (line && *line) {
            ...
            /*
             * 格式一般是：
             * ffffffc010123456 T symbol_name
             */
            {
...
static int hwBreakpointProc_dev_init(void)
{
    ...
    unsigned long kallsyms_lookup_name_addr = 0;
    unsigned long sys_call_table_addr = 0;

    kallsyms_lookup_name_addr =
        lookup_symbol_from_kallsyms("kallsyms_lookup_name");

    sys_call_table_addr =
        lookup_symbol_from_kallsyms("sys_call_table");

    printk(KERN_EMERG
        "[ctx] kallsyms_lookup_name=%px sys_call_table=%px\n",
        (void *)kallsyms_lookup_name_addr,
        (void *)sys_call_table_addr);

    if (!kallsyms_lookup_name_addr) {
        printk(KERN_EMERG
            "[ctx] kallsyms_lookup_name unavailable, "
            "symbol-dependent features disabled\n");
    }

    if (!sys_call_table_addr) {
        printk(KERN_EMERG
            "[ctx] sys_call_table unavailable\n");
    }
